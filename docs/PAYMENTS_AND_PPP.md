...
> [ROADMAP.md](../ROADMAP.md). The reference implementation is the AgapeVerse
> rewrite (`~/_Projects/AgapeVerse/agapeverse-nebulakit`); paths below point at the
> working code to lift and generalize.
...
## How AgapeVerse does it (reference map)

| Concern                                                                                                                                                                     | File                                                                      | Lift as                                                                                                 |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Country→currency map, currency list, zero-decimal / whole-unit display rules, `formatMoney`, `toMajorUnits`, preview-override resolution, **PPP amount table**              | `src/lib/utils/region-pricing.ts` (536 lines)                             | Core of a kit `region-pricing` module — split the domain-specific amount table out (see "Generalizing") |
...
| Live-FX USD reference for the admin preview (sanity-check that ₹479 ≈ $5.7 PPP vs the $9.99 tier)                                                                           | `src/lib/utils/fx.ts` (key-less open.er-api.com, KV-cached, preview-only) | Kit util, optional                                                                                      |
...
5. **Charge.** The resolved `currency` is passed straight into the Checkout Session
   (`createSubscriptionCheckoutSession` / `createHeartpointsCheckoutSession`). Stripe
   charges the `currency_options` amount. Webhooks and entitlements never see it.

## Generalizing for the kit (the real work)

AgapeVerse's table is domain-specific: `PriceRole = 'monthly' | 'yearly' | 'hp100' |
'hp500' | 'hp1500'`, with HeartPoint packs scaling by full PPP tier and the
subscription floored at ~$4.99/$49.99 USD-equivalent (because a Premium subscriber's
included allowance can cost that much to serve). The kit can't ship HeartPoints as a
concept in the pricing table.

Plan to split it into two layers:

1. **Currency & formatting infrastructure — lift almost verbatim.** The country map,
   ...
   the bulk of `region-pricing.ts`. It belongs in the kit unchanged.
2. **The PPP amount table — make it the downstream app's config.** Replace the fixed
   ...
   currencies like TRY/NGN). Ship AgapeVerse's numbers as a **worked example**, not as
   the kit's prices.
...
The credits/points ledger (HeartPoints, generalized) is a **separate** roadmap module.
It _consumes_ this pricing layer (its packs are just price roles) but PPP must not
depend on it — keep the dependency one-directional.
...
- **Live-key safety.** The setup script refuses live-mode keys without an explicit
  ...
  share a Stripe account across projects (AgapeVerse does) need the freeze to be
  loud.
- **Currency the card can't pay.** Only map countries to a currency their cards
  actually support (AgapeVerse maps home countries + official-use states, e.g.
  Liechtenstein→CHF, EUR micro-states). Everything else → USD. Presenting a currency
  the visitor can't be charged in is worse than USD.
...
- **Zero-decimal & whole-unit correctness.** ¥1500 means 1500, not 150000; HUF/TWD
  ...
  `formatMoney`/`toMajorUnits` and their tests (`region-pricing.test.ts`) must come
  over together.
...
- **Amount-derivation helper — documented manual method + AgapeVerse's numbers as a
...
- **Currency breadth — keep AgapeVerse's full 46-currency list as the kit default.**
  It's already built, tested, and covers the markets where PPP actually matters;
  trimming to a "core 15" would discard working code and weaken the default. Document
  the country→currency map as the trim/extend point for apps that want fewer.
