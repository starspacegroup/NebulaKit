...
### Migrations: a `0010_` collision that is still fixable

`migrations/` is D1-native — wrangler sorts `.sql` files by numeric prefix and then by string, and records applied ones in `d1_migrations` **by filename**. A filename already on `main` is already applied and is immutable (AGENTS.md §2, `migrations/README.md`).

Two files share the `0010_` prefix, and ordering between them falls to the string comparison, so `0010_content_item_timestamp_proof.sql` runs before `0010_oauth_transactions.sql`. Only the first is on `origin/main`; `0010_oauth_transactions.sql` and `0011_minimize_oauth_tokens.sql` are **branch-only and have never been applied to a remote D1**, so the immutability rule does not yet bind them. Renaming those two to `0011_`/`0012_` before the branch merges removes the collision entirely, and is preferable to shipping a duplicate number into `main` where it would permanently weaken §2's "next sequential migration" rule for every sibling that pulls this file. Both are safe to replay (`0010_` guards with `IF NOT EXISTS`; `0011_` is an idempotent `UPDATE`), though a local D1 that already applied them will re-run them under the new names. If the branch merges as-is instead, the collision becomes permanent and the next sequence number is `0013_`.
